---
description: "Odoo and OCA development standards: module structure, OCA guidelines, migration, pre-commit, odootest, security. Use when working on Odoo modules, __manifest__.py, OCA contributions, or Odoo XML/Python files."
applyTo: "**/{__manifest__.py,__openerp__.py}"
---

# Odoo Development Context

Always provide code that meets OCA standards, is community-ready, well-tested, and production-grade.
You are developing Odoo modules following enterprise-grade standards and OCA contribution guidelines. Always adhere to these guidelines:
...
- **One model class per file** — file name must match the model name (e.g., `sale_order.py` for `sale.order`)
...
- Minimize `setUpClass` / `setUp` data: create only what the current test class needs; leverage parent common test classes (e.g., `TestSaleCommon`, `TestStockCommon`) for shared fixtures
- Group related test methods in a single test class; split into separate files/classes when testing distinct features
- Use `cls.env["model.name"]` in `setUpClass` and `self.env["model.name"]` in test methods — never mix them
- Method ordering within a model class: `_name`/`_inherit`/`_description` → fields → constrains/onchanges → CRUD overrides → business methods → private helpers
...
- Create a README.rst if not exists, following this formalism:
  - The title is rounded by `=` signs
  - A brief explanation of the module's purposes
...
- This includes:
  ...
  - security files
  - static files
- Provide a migration guide in README.rst if necessary
- Provide a migration script if necessary
- When changes are made, inform the user that the job has been done and needs review
- Once validated, add to the staging area in git and proceed to commit which must have this formalism:
  - `[MIG] {module name}: migration to XX.0` where XX is the major Odoo version being migrated
...
- Use descriptive test method names starting with `test_` and easily readable for humans (short sentence explaing what the test is for)
- Use exhaustive and descriptive docstring to give hints of what the test method is used for
- Ensure tests are isolated and repeatable
- Use `odootest -o <module name>` to install/update a module with its dependencies and run Odoo tests
...
- DON'T write comments that state the obvious — only useful comments that explain *why* a piece of code exists, not *what* it does (reading the code suffices)
- DON'T use `"""` docstrings when overriding a method — use `#` comments instead
