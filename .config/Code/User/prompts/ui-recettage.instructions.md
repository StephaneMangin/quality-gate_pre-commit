---
description: "UI recettage harness standards: pytest-playwright, page objects, cursor overlay, MP4 artefacts, Odoo-version agnostic selectors. For scripts/ui_recettage/ or any Playwright test against an Odoo web UI."
applyTo: "**/scripts/ui_recettage/**"
---

# UI Recettage Harness (Odoo, pytest-playwright)

Objective: drive an Odoo web UI as a human operator (see, click, validate) and produce human-watchable evidence (MP4 videos, screenshots, filtered server log, trace.zip) per run. All guidance below is intentionally agnostic of the Odoo major version — never hard-code a version number.

## Layout (MANDATORY)

```
scripts/ui_recettage/
...
- **Never run pytest without `-p no:odoo`** when Odoo's pytest plugin is installed in the same venv — it auto-loads the module and corrupts collection. The `pytest.ini` in this folder must keep `-p no:odoo` or run it explicitly.
- **Never use `page.wait_for_load_state("networkidle")`** against Odoo — the bus/websocket long-poll prevents idle. Use DOM anchors: `.o_main_navbar`, `.o_action_manager`, `.o_barcode_client_action`, or a screen-specific selector, via `wait_web_client_ready()` helper.
...
- **Never expose private RPC from JS.** If a scenario requires a server-side helper, add a public method on the Odoo model and call it via public RPC. Document the public alias.
- **Never commit `artifacts/`** — it is gitignored. Also ignore `__pycache__`, `.pytest_cache`.
- **Never keep `.webm`** in the final run output — they are auto-converted to `.mp4` by the session teardown in `conftest.py`. MP4 is the only human-distributable format.

## Odoo-version agnosticism

The harness **must** work across Odoo major versions without code change. Enforce this by:

1. **Detect the version dynamically** at fixture time. Preferred order:
   - Read `odoo.release.version_info` when the `odoo` Python package is importable in the test venv.
   - Else parse the project's config (`odoo-ci.cfg` / `odoo.cfg`) and/or the manifest of any addon (`__manifest__.py` `"version"` key: e.g. `"18.0.1.0.0"` → major `18`).
   - Else fall back to the `ODOO_VERSION` env var (int, e.g. `18`).

   Expose it as `UIConfig.odoo_version: int` and as a pytest fixture `odoo_version`.

2. **Branch selectors, not behaviour.** When a selector changes across versions, create a helper in `core/waits.py` or a small `core/selectors.py` module:
   ```python
   ...
   ```
   Tests call the helper; never inline version checks in `tests/`.

3. **Page objects receive `UIConfig`** (already do). They must read `config.odoo_version` when building version-sensitive locators. Keep the dispatch local to the page object.
...
- `authenticated_page` — returns a logged-in Playwright `Page`.
- `recorder` — call `recorder.step("label")` at every meaningful UI checkpoint; it captures a screenshot and appends to `summary.json`.
- `server_log` — filters the Odoo log to only the lines emitted during the test body (requires `ODOO_LOG_FILE` env).
- `odoo_version` — integer major version; inject it into page objects.

## Video quality (already configured)

- `UI_RECETTAGE_SLOWMO_MS` default = **400 ms** — gives a watchable rhythm.
- Cursor overlay is auto-injected via `context.add_init_script(CURSOR_INIT_SCRIPT)` in an `autouse` fixture. It draws a red dot that follows `mousemove/down/up` with CSS transitions so the cursor *glides* between targets and emits a ripple on click.
- Session teardown transcodes every `*.webm` → `*.mp4` (H.264, yuv420p, faststart) via `ffmpeg`, then deletes the source `.webm`. If `ffmpeg` is absent the webm is kept; add `ffmpeg` to the dev image.
...
| `UI_RECETTAGE_HEADED`    | `0`                      | `1` to show Chromium                         |
| `UI_RECETTAGE_SLOWMO_MS` | `400`                    | Delay between Playwright actions             |
| `UI_RECETTAGE_ARTIFACTS` | `artifacts/ui_recettage` | Root dir for run artefacts                   |

## Adding a scenario (TDD)

1. Write or extend a **page object** under `pages/` with `:visible`-filtered locators and a version-aware helper when needed.
2. Add a **seeder** under `scenarios/` (or a thin wrapper shelling out to `scripts/<addon>/setup_*.py`). Return a frozen dataclass with ids + names.
...
4. Implement the page object method until the test passes.
5. Run the suite: `pytest -p no:odoo scripts/ui_recettage -q`. Watch the generated MP4 to validate the rhythm and the click targets.
6. Pre-commit: suite must be ruff/black/isort clean. Per-file ignores for the harness live in the project `pyproject.toml`.

## Artefact layout (per run)

```
artifacts/ui_recettage/run_YYYYMMDD_HHMMSS/
  <test-node-id>/
...
## Reviewing a recettage run

- Open `summary.json` to understand the step timeline and failure reason.
- Watch the MP4 in VLC / browser at real speed — if any action is inaudible/invisible, bump the slowmo or add a `recorder.step()` right before it.
- Correlate with `server_log_filtered.log` for backend errors (AccessError, ValidationError, private-method RPC rejection).

## Reference implementation skeleton

This is the **single source of truth** for the harness internals. Any new workspace must reproduce these contracts byte-for-byte (module names, fixture names, return types). Deviations break the rest of the instructions.
...
```
scripts/ui_recettage/artifacts/
scripts/ui_recettage/.pytest_cache/
scripts/ui_recettage/**/__pycache__/
```
...
"scripts/ui_recettage/**" = ["S101", "S603", "E501"]
```
...
class UIConfig:
    ...
    def from_env(cls) -> "UIConfig":
        artifacts = Path(
            os.environ.get("UI_RECETTAGE_ARTIFACTS", "artifacts/ui_recettage")
        ).resolve()
        ...
        return cls(
            ...
            headed=_env_bool("UI_RECETTAGE_HEADED", False),
            slowmo_ms=_env_int("UI_RECETTAGE_SLOWMO_MS", 400),
...
(() => {
    if (window.__uiRecettageCursorInstalled) return;
    window.__uiRecettageCursorInstalled = true;
    const install = () => {
        if (!document.body) return requestAnimationFrame(install);
        const cursor = document.createElement('div');
        cursor.id = '__ui-recettage-cursor';
...
`filter_lines()` applies an `|`-joined regex against each line. Default patterns: `Traceback`, `\bERROR\b`, `Access Error`, plus project-specific keywords (pass a custom tuple if needed).
...
One location for every known selector divergence across Odoo versions. Add a function when you hit a real divergence; do **not** preempt with speculative versioning.
...
**Rule:** every version branch must cite the Odoo release where the change landed (commit, release notes, or forum post) in a comment adjacent to the `if odoo_version >= N:`.
...
When adding a version branch, append a row here **before** touching code.
...
Keep the matrix accurate; it is the canonical changelog for harness version adapters.

## Checklist when porting the harness to a new workspace

1. Copy `scripts/ui_recettage/` skeleton (`core/`, `pages/login_page.py`, `conftest.py`, `pytest.ini`, `README.md`).
2. Add `.gitignore` entries and `pyproject.toml` snippets above.
3. `pip install -e '.[test]' && playwright install chromium` ; install `ffmpeg` on the host.
4. Set `ODOO_LOG_FILE` in your Odoo runner to the file consumed by `server_log`.
5. Write the first seeder + test; run `pytest -p no:odoo scripts/ui_recettage -q`.
6. Inspect the first MP4 to validate cursor visibility and rhythm; tune `UI_RECETTAGE_SLOWMO_MS` if needed.
