...
def _load_module_manifests(
    ...
    """Load all module manifests. Returns {module_name: {name, depends, version}}."""
    graph = {}
    max_modules = 2000
    ...
    for manifest_path in _iter_with_progress(
...
def _build_dependency_graph(manifests: dict[str, dict]):
    """Build a networkx directed graph from module manifests.
...
def _compute_dependency_metrics(manifests: dict[str, dict], graph=None) -> dict:
    """Compute dependency graph metrics with networkx or fallback calculations."""
    metrics = {
